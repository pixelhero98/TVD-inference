...
CANONICAL_TVD_SCHEDULER_KEY = "tvd"
DEFAULT_SIGNAL_TRACE_KEY = CANONICAL_INFO_GROWTH_TRACE_KEY
...
def collect_forecast_calibration(
    ...
    *,
    ...
    for example_idx in range(len(ds_val)):
        hist_t, _, _, _ = ds_val[int(example_idx)]
        ...
        for sample_idx in range(trace_samples):
            ...
            rollout_grid = trace["time_grid"].detach().cpu().numpy().astype(np.float64)
            if reference_time_grid is None:
                reference_time_grid = rollout_grid
            elif not np.allclose(reference_time_grid, rollout_grid, atol=1e-8, rtol=1e-8):
        ...
        disagreement_trace = np.stack(disagreement_samples, axis=0).mean(axis=0)
        residual_trace = np.stack(residual_samples, axis=0).mean(axis=0)
        oracle_trace = np.stack(oracle_samples, axis=0).mean(axis=0)
        disagreement_rows.append(disagreement_trace)
        residual_rows.append(residual_trace)
        oracle_rows.append(oracle_trace)
        for step_idx, (disagreement_value, residual_value, oracle_value) in enumerate(
            zip(disagreement_trace.tolist(), residual_trace.tolist(), oracle_trace.tolist())
        ):
            trace_rows.append(
                {
    ...
    if not disagreement_rows:
        raise ValueError("Forecast validation split is empty; cannot calibrate canonical TVD schedule.")
    ...
    base_r_star = resolved_r_star(residual_arr.reshape(-1))
    ...
    canonical_arr = compute_canonical_info_growth_hardness_numpy(
        residual_arr,
        disagreement_arr,
        r_star=float(effective_r_star),
    )
    no_rstar_arr = compute_no_rstar_info_growth_hardness_numpy(residual_arr, disagreement_arr)
    corr_signal = canonical_arr[:, 1:].reshape(-1)
    corr_no_rstar = no_rstar_arr[:, 1:].reshape(-1)
    ...
    return {
        ...
        "solver": str(solver_name),
        "n_windows": int(canonical_arr.shape[0]),
        ...
        CANONICAL_INFO_GROWTH_TRACE_KEY: [float(x) for x in canonical_arr.mean(axis=0).tolist()],
        EXPERIMENTAL_NO_RSTAR_INFO_GROWTH_TRACE_KEY: [float(x) for x in no_rstar_arr.mean(axis=0).tolist()],
        "signal_correlations_vs_oracle": {
            CANONICAL_INFO_GROWTH_TRACE_KEY: {"spearman": safe_spearman(corr_signal, corr_oracle)},
            EXPERIMENTAL_NO_RSTAR_INFO_GROWTH_TRACE_KEY: {"spearman": safe_spearman(corr_no_rstar, corr_oracle)},
        },
    }
